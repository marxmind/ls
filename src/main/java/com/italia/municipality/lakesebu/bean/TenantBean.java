...
public class TenantBean implements Serializable {
	...
	private int yearSelectedSearch;
	private List yearsSearch;
	private int monthSelectedSearch;
	private List monthsSearch;
	...
	public void loadBillCreated() {
		...
		*/
		...
		billTenants = TenantBilling.getTenantBillLatest(getYearSelected(), getMonthSelected(), name);
	...
	public void delete(Tenant ten) {
		boolean hasContract = TenantContract.hasExistingContract(ten.getId());
		if(!hasContract) {
			ten.delete();
			load();
			Application.addMessage(1, "Success", "Successfully deleted");
		}else {
			Application.addMessage(2, "Warning", "This tenant has an existing contract");
		}
	}
	...
	public void deleteContract(TenantContract ten) {
		boolean hasBill = TenantBilling.hasExistingBills(ten.getTenant().getId(),ten.getId());
		if(!hasBill) {
			ten.delete();
			loadContract();
			Application.addMessage(1, "Success", "Successfully deleted");
		}else {
			Application.addMessage(2, "Warning", "This contract cannot be deleted. This contract has an existing bill");
		}
	}
	...
	public void loadBill() {
		...
		if(getYearSelectedSearch()>0) {
			sql += " AND bill.yer=" + getYearSelectedSearch();
		}
		
		if(getMonthSelectedSearch()>0) {
			sql += " AND bill.mnt=" + getMonthSelectedSearch();
		}
...
public void printBillIndividual(TenantBilling py) {
		...
		//for(TenantBilling py : TenantBilling.retrieve(sql, new String[0])) {
			...
			int yearNow = py.getYear();
			long contractId = py.getContract().getId();
			
			if(billDataContract!=null) {
				if(billDataContract.containsKey(contractId)) {
					if(billDataContract.get(contractId).containsKey(yearNow)) {
						billDataContract.get(contractId).get(yearNow).add(bll);
					}else {
						billList = new ArrayList<Bills>();
						billList.add(bll);
						billDataContract.get(contractId).put(yearNow, billList);
					}
				}else {
					billDataYear = new LinkedHashMap<Integer, List<Bills>>();
					billList = new ArrayList<Bills>();
					billList.add(bll);
					billDataYear.put(yearNow, billList);
					billDataContract.put(contractId, billDataYear);
				}
			}else {
				billList.add(bll);
