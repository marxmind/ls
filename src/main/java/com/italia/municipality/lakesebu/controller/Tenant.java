...
public class Tenant {
	...
	public static String getNewTenantNumber() {
		...
		ps = conn.prepareStatement("SELECT tenantno FROM tenant ORDER BY tid DESC LIMIT 1");
		rs = ps.executeQuery();
		
		while(rs.next()){
			String[] val = rs.getString("tenantno").split("-");
 			yearLast = val[0];
	...
	public static List<Tenant> retrieve(String sql, String[] params){
		List<Tenant> tns = new ArrayList<Tenant>();
		
		String sqlTemp = "SELECT * FROM tenant WHERE isactivet=1 ";
		...
		System.out.println("Tenant SQL " + ps.toString());
		rs = ps.executeQuery();
	...
	public static Tenant save(Tenant st){
		if(st!=null){
			
			long id = Tenant.getInfo(st.getId() ==0? Tenant.getLatestId()+1 : st.getId());
			LogU.add("checking for new added data");
			if(id==1){
				LogU.add("insert new Data ");
				st = Tenant.insertData(st, "1");
			}else if(id==2){
				LogU.add("update Data ");
				st = Tenant.updateData(st);
			}else if(id==3){
				LogU.add("added new Data ");
				st = Tenant.insertData(st, "3");
			}
	...
	public static Tenant insertData(Tenant st, String type){
		...
		int cnt = 1;
		LogU.add("===========================START=========================");
		LogU.add("inserting data into table tenant");
		...
		ps.setString(cnt++, st.getTenantNo());
		...
		LogU.add("data has been successfully saved...");
			...
			LogU.add("error inserting data to tenant : " + s.getMessage());
		}
		LogU.add("===========================END=========================");
		return st;
	}
	
	public static Tenant updateData(Tenant st){
		...
		int cnt = 1;
		LogU.add("===========================START=========================");
		LogU.add("updating data into table tenant");
		...
		LogU.add("data has been successfully saved...");
			...
			LogU.add("error updating data to tenant : " + s.getMessage());
		}
		LogU.add("===========================END=========================");
		return st;
	}
	...
	public static long getLatestId(){
		...
		String sql = "";
		try{
		sql="SELECT tid FROM tenant ORDER BY tid DESC LIMIT 1";	
		conn = WebTISDatabaseConnect.getConnection();
	...
	public static boolean isIdNoExist(long id){
		...
		try{
		conn = WebTISDatabaseConnect.getConnection();
		ps = conn.prepareStatement("SELECT tid FROM tenant WHERE tid=?");
		ps.setLong(1, id);
		rs = ps.executeQuery();
	...
	public  void delete(){
		...
		Connection conn = null;
		PreparedStatement ps = null;
		String sql = "UPDATE tenant set isactivet=0 WHERE tid=?";
